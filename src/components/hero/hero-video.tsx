...
export function HeroVideo({
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden">
      ...
      {/* Video Background (overlays poster when loaded) */}
      <video
        autoPlay
        muted
        loop
      ...
      <div className="relative z-10 flex h-full flex-col items-center justify-end pb-24 text-center text-white">
        <h1 className="mb-6 max-w-3xl px-6 font-serif text-5xl font-light leading-tight md:text-7xl animate-fade-in">
          {headline}
        </h1>

        <button
          onClick={scrollToNext}
          className="nav-text mb-8 cursor-pointer border border-white/60 px-8 py-3 text-white/90 transition-all duration-400 hover:bg-white hover:text-black animate-slide-up"
        >
          {ctaText}
        </button>
